...
The package is aarch64-only and published to edge only. It widens to rc and stable only after M1 and M2 cold-boot qualification. It depends only on generic packages, and its only provides are the retired Apple-only names, so nothing generic can pull it onto non-Apple aarch64 machines.
